...
export default class Chart extends React.Component<IChartProps, IChartState> {
  ...
  public chartData(): any {
    ...
    //     )
    //   ).sort((a:any, b:any) => a - b);  // Sort years
    
    //   data.labels = years;  // Set x-axis labels (years)
    
    //   // Create datasets for each investment name (each line on the chart)
    //   data.datasets = Array.from(map.keys()).map((investmentName:any, i:number) => {
    //     const investmentData = map.get(investmentName);
    
    //     return {
    //       label: investmentName,  // Investment name (label for the line)
    //       data: years.map((year:any) => investmentData[year] || 0),  // Sum investment amounts for each year
    ...
    // }
      ...
      let yearMap = new Map();  // To store total investment per year
    
      // Iterate through the items to group and sum by year
      this.state.items.forEach((item) => {
        const year = item["Investment_Date"];       // Investment Date (Year)
        const investmentAmount = item["field_3"];   // Investment Amount
    
        if (!year || investmentAmount === undefined) {
        ...
        yearMap.set(year, yearMap.get(year) + investmentAmount);
      });
    
      // Get sorted list of years
      const years = Array.from(yearMap.keys()).sort((a: any, b: any) => a - b);
      data.labels = years;
    
      // Create a single dataset for total investment
      const totalInvestments = years.map((year: any) => yearMap.get(year) || 0);
    
      data.datasets.push({
        label: "Total Investment",
        data: totalInvestments,
        backgroundColor: this.props.colors[0],   // Use the first color
